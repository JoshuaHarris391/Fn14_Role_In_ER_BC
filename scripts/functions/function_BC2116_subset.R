...
BC2116_subset <- function(INPUT_DF, CLIN_DF, OUTPUT_CLIN, INPUT_GENES, RETURN_PROBES = F){
  ...
  # INPUT_DF <- EXP
  # CLIN_DF <- CLIN
  # INPUT_GENES = c("TNFRSF12A", "CDH1", "TP53", "ZEB1", "VIM")
  # OUTPUT_CLIN = T
  
  # Adding gene name to input_df
  m <- match(rownames(EXP), ANOT_DF$PROBES)
  INPUT_DF$GENE <- ANOT_DF[m, "GENES"]
  INPUT_DF$PROBES <- ANOT_DF[m, "PROBES"]
  ########################################
  ...
  PLOT_DF <- INPUT_DF[m, ] %>% 
    dplyr::select(., !(GENE) & !(PROBES)) %>% 
    t() %>% 
    as.data.frame()

  
  # Subset PLOT_DF by gene, and find probe with highest avg expression, if there is more than one probe
  optimal_probe <- list()
  for (gene_name in found_genes) {
    # extract probes for gene iteration
    probes <- probe_list[[gene_name]]
    
    # Select probe with highest avg expression of there are multiple probes
    if(length(probes) > 1){
      # select probes
      ...
      optimal_probe[[paste0(gene_name)]] <- probes
    }
  ...
  if (OUTPUT_CLIN == T) {
    ...
    PLOT_DF <- dplyr::select(PLOT_DF, !(GENE) & !(PROBES))
  }
  ...
  if (OUTPUT_CLIN == T) {
    # Adding pateint ID
    PLOT_DF$ID_REF <- rownames(PLOT_DF) %>% factor(., levels = CLIN_DF$ID_REF)
    # Combining Clinical DF with Plot DF
    PLOT_DF <- inner_join(PLOT_DF, CLIN_DF, by = "ID_REF")
  }
