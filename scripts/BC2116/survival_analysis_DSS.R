...
KM_Plot_List <- list()
KM_Plot_List[["ER_Pos"]] <- GENE_EXP_ER_P %>% dplyr::select(DSS_TIME, DSS_EVENT, Fn14_TERT_GROUP, Fn14_TERT)
KM_Plot_List[["PR_Pos"]] <- GENE_EXP_PR_P %>% dplyr::select(DSS_TIME, DSS_EVENT, Fn14_TERT_GROUP, Fn14_TERT)
KM_Plot_List[["HR_Pos"]]<- GENE_EXP_HR_P %>% dplyr::select(DSS_TIME, DSS_EVENT, Fn14_TERT_GROUP, Fn14_TERT)
KM_Plot_List[["HR_Pos_Endo_Mono_T"]]<- GENE_EXP_HR_P_Endo_Mono_T %>% dplyr::select(DSS_TIME, DSS_EVENT, Fn14_TERT_GROUP, Fn14_TERT)
KM_Plot_List[["HR_Pos_Chemo_Endo"]]<- GENE_EXP_HR_P_Chemo_Endo %>% dplyr::select(DSS_TIME, DSS_EVENT, Fn14_TERT_GROUP, Fn14_TERT)
...
for (i in names(KM_Plot_List)) {
  ...
  SURV <- Surv(DSS_TIME, DSS_EVENT) ~ Fn14_TERT_GROUP
  FIT <- survival::survfit(Surv(DSS_TIME, DSS_EVENT) ~ Fn14_TERT_GROUP, data = INPUT_DF)
  
  # # Saving 10 year survival 
  # year_time <- 10
  # tenyearsurv <- summary(FIT, times = year_time) %>% capture.output()
  # dir.create(path = "outputs/BC2116/KMplots/DSS/ten_year_surv_summary", recursive = T, showWarnings = F)
  # survfilepath <- paste("outputs/BC2116/KMplots/DSS/ten_year_surv_summary/", i, "_ten_year_surv.txt", sep = "")
  # write_lines(tenyearsurv, file = survfilepath)
  
  # Calculating univar cox
  Cox_Uni <- coxph(Surv(INPUT_DF$DSS_TIME, INPUT_DF$DSS_EVENT) ~ INPUT_DF[, "Fn14_TERT"], data = INPUT_DF)
  # Prepare the columns
  beta <- coef(Cox_Uni)
  HR  <- exp(beta)
  ...
  KMplot <- ggsurvplot(
    ...
    pval = HR_Pval_lab,
    pval.size = 3.9,
    pval.coord = c(max(DSS_TIME)*0.48 ,0.95),
    legend.title = "",
    legend.labs = KM_LEDG,
    legend = c(0.23,0.2),
    risk.table = F,
...
##########################################
# Variables
dir.create("outputs/BC2116/KMplots/DSS/", recursive = T, showWarnings = F)
File.Path <- "outputs/BC2116/KMplots/DSS/"
