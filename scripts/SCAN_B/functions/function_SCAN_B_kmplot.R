########################################
# Function to produce KMplot
########################################
SCAN_B_kmplot <- function(INPUT_DF, INPUT_GENE, CLIN_SUBSET_VAR, CLIN_SUBSET_FACTOR, LEG_POS, QUANTILE_TYPE){
  ...
  # Creating multivar Cox Model 
  SURV.int.uni <- Surv(PLOT_DF$OVERALL_SURVIVAL_MONTHS, PLOT_DF$OVERALL_SURVIVAL_EVENT) ~ 
    PLOT_DF[, INPUT_GENE] + PLOT_DF$ER_STATUS + PLOT_DF$PGR_STATUS + PLOT_DF$HER2_STATUS
  cox.int.uni <- coxph(SURV.int.uni, data = PLOT_DF)
  # pulling cox stats
  HR.int.uni <-summary(cox.int.uni)$coefficients[1, 2]
  p.value.int.uni <- summary(cox.int.uni)$coefficients[1, 5]
  Cox.graph.int.uni <- paste("Multivariate Cox:  ", "HR Ratio", "=", format(HR.int.uni, digits = 4), "   ", "P", "<", format(p.value.int.uni, digits = 4)) 
